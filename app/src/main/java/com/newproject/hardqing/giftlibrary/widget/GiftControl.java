...
//        loadGift(gift, true,);
    ...
    public interface EndGiftCountListener {

        void dismiss(int count, String giftId, String giftname);
    }

    EndGiftCountListener endGiftCountListener;

    public void setEndGiftCountListener(EndGiftCountListener listener) {
        endGiftCountListener = listener;
    }
...
//    volleyNums 连击数  多想想这些参数哪里来的
    public void loadGift(GiftModel gift, boolean supportCombo) {
        ...
        if (mGiftQueue != null) {
            if (supportCombo) {
                ...
                for (int i = 0; i < mGiftLayoutList.size(); i++) {
                    giftFrameLayout = mGiftLayoutList.get(i);
                    if (gift.getVolleyNums() > 0) {
                        giftFrameLayout.setVolleyNums(gift.getVolleyNums());
                    }
                    if (giftFrameLayout.isShowing()) {
                        if (giftFrameLayout.getCurrentGiftId().equals(gift.getGiftId()) && giftFrameLayout.getCurrentSendUserId().equals(gift.getSendUserId())) {
                            ...
                            if (gift.getVolleyNums() > 0) {
                                ...
                                giftFrameLayout.setGiftCount(gift.getGiftCount() + gift.getVolleyNums());
                            } else {
     ...
     * @param gift
     * @param supportCombo 是否支持实时连击，如果为true：支持，否则不支持
     * @param volleyNums 连击数
     */
    public void loadGift(GiftModel gift, boolean supportCombo, int volleyNums) {
        ...
        if (mGiftQueue != null) {
            if (supportCombo) {
                ...
                for (int i = 0; i < mGiftLayoutList.size(); i++) {
                    giftFrameLayout = mGiftLayoutList.get(i);
                    giftFrameLayout.setVolleyNums(volleyNums);
                    if (giftFrameLayout.isShowing()) {
                        if (giftFrameLayout.getCurrentGiftId().equals(gift.getGiftId()) && giftFrameLayout.getCurrentSendUserId().equals(gift.getSendUserId())) {
                            //连击
                            Log.i(TAG, "addGiftQueue: ========giftFrameLayout(" + giftFrameLayout.getIndex() + ")连击========礼物：" + gift.getGiftId() + ",连击X" + gift.getGiftCount());
    ...
    public void dismiss(int index) {
        ...
        for (int i = 0; i < mGiftLayoutList.size(); i++) {
            ...
            if (giftFrameLayout.getIndex() == index) {
                endGiftCountListener.dismiss(giftFrameLayout.getGiftCount(), giftFrameLayout.getCurrentGiftId(),giftFrameLayout.getGiftPic());
